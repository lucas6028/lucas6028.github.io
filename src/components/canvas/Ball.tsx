...
interface BallProps { imgUrl: string; }
...
const Ball = (props: BallProps) => {
  ...
  return (
    ...
    <Float speed={1.75} rotationIntensity={1} floatIntensity={2}>
      <ambientLight intensity={0.25} /> {/* Ambient light for general illumination */}
      <directionalLight position={[0, 0, 0.05]} /> {/* Directional light for shading */}
      <mesh receiveShadow castShadow scale={2.75}> {/* Mesh for the ball */}
        <icosahedronGeometry args={[1, 1]} /> {/* Geometry of the ball */}
        <meshStandardMaterial color={"#fff8eb"} polygonOffset polygonOffsetFactor={-5} flatShading /> {/* Material of the ball */}
        <Decal position={[0, 0, 1]} rotation={[2 * Math.PI, 0, 6.25]} map={decal} /> {/* Decal texture on the ball */}
      </mesh>
    </Float>
  )
}
...
const BallCanvas = ({ icon }: BallCanvasProps) => {
  return (
    ...
    >
      <Suspense fallback={<CanvasLoader />}> {/* Suspense for lazy loading */}
        <OrbitControls
          enableZoom={false}
        /> {/* Orbit controls for rotating the view */}
        <Ball imgUrl={icon} /> {/* Render the Ball component with the provided icon */}
      </Suspense>
